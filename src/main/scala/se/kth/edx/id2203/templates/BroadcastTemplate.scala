...
// FIXED: Changed from 'case class' to 'class' to allow inheritance from RB_Broadcast
  // Also fixed the parameter name from 'p' to 'payload' to match the constructor
  ...
  beb uponEvent {
    case BEB_Deliver(src, y @ RB_Broadcast(payload)) => {
        ...
        if(!delivered.contains(payload)){
          ...
          trigger(BEB_Broadcast(DataMessage(src, y.payload)) -> beb);*/
          ...
          val originalSource = y match {
            ...
            // Re-trigger the same message to ensure Agreement
...
class WaitingCRB(init: Init[WaitingCRB]) extends ComponentDefinition {
  ...
  crb uponEvent {
    case x: CRB_Broadcast => {
        /* WRITE YOUR CODE HERE */
        val W = VectorClock(vec); // Use the copy constructor
        W.set(self, lsn); 
        lsn = lsn + 1;
  ...
  rb uponEvent {
    case x @ RB_Deliver(src: Address, msg: DataMessage) => {
      ...
      while (deliverableFound) {
        // Look for any message in pending that satisfies the causal order
        val nextMsg = pending.find { case (p_prime, data) => data.timestamp <= vec };
        
        nextMsg match {
          case Some(entry @ (p_prime, data)) => {
            // Line 13: pending := pending \ {(p', W', m')}
            pending -= entry;
            // Line 14: V[p'] := V[p'] + 1
            vec.inc(p_prime);
            // Line 15: trigger < crb, Deliver | p', m' >
            trigger(CRB_Deliver(p_prime, data.payload) -> crb);
          }
          case None => {
            // No more deliverable messages at this time
